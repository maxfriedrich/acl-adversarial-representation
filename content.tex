...
may be replaced by
\begin{quote}
    [Henry] was admitted to [Chapel Davis].
\end{quote}
%
% TODO the sentence does not make sense but the embeddings are still similar
...
\begin{description}
    ...
    % TODO probably throw this section away?
    \item[Training, Validation, and Test Set]
    %
    While developing the models, we use a fixed, relatively large validation set.
    %
    At test time, we combine the training and validation set and use a smaller validation split.
    %
    We use the test set only for the final evaluation of each model. 
\end{description}
...
%
In this section, we discuss the de-identification performance and privacy properties, and generalization abilities of our models.
...
\begin{description}
    ...
    %
    Slight misspellings have embeddings close to the correct spelling in FastText.
    %
    Also, entity names that clearly belong to a specific category (like city names with the suffix ``-town'') have meaningful embeddings in FastText.
    %
    Some of the FastText models' mistakes could be avoided by using a dictionary of professions, hospital names, and medical terms as an additional data source, e.g.\ ``Bruce Protocol'' is a standard cardiac diagnostic test that would occur in a medical dictionary.
