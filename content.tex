...
automatically finding these instances helps manual annotators to then replace the \ac{phi} with
non-identifying replacements.  As the information to be removed is very sensitive, the sanitization
cannot be fully automated -- the automatic labeling serves to speed up the process.
...
by transforming text non-reversible into a vector space and only sharing that representation.
%
...
%
Our work builds upon two lines of research: first, de-identification,
as the system has to provide good de-identification performance, and
second adversarial representation learning, to remove all identifying
...
%
The adversary and the classifier are neural networks which share the first layers and the system
uses a gradient reversal layer to worsen the representation for the adversary during back-propagation,
i.\,e.\ when training the adversary, the adversary-specific part of the network is optimized for the
adversary but the shared part is updated against the gradient to make the shared representation less suitable
for the adversary.
...
% Hinweis: Solche Sätze findet man häufiger um eine leere Section zu vermeiden, aber der Satz
% gibt nicht viel mehr Information als die Überschrift selbst.  Deshalb:  Mehr schreiben :-)
% In this section, we describe the i2b2 2014 dataset as well as our experimental approaches.

%TODO

\section{Dataset and de-identification model}
